...
local function safeid(s)
    if not s or not s:match("^[%w_%-]+$") then
        return ""
    end
    return s
end
...
local function safepath(s)
    ...
    if s:sub(1, 1) ~= "/" or s:find("//", 1, true) or s:find("%.%.") then
        return ""
    end
    if s:find("[<>\"']") then
        return ""
...
local function shell(o)
    ...
    if o.font ~= "" then
        rules[#rules + 1] = "--body: " .. csssafe(o.font)
    end
    if o.measure ~= "" then
        rules[#rules + 1] = "--measure: " .. csssafe(o.measure)
    end
    if o.ink ~= "" then
        rules[#rules + 1] = "--ink: " .. csssafe(o.ink)
    end
    if o.paper ~= "" then
        rules[#rules + 1] = "--paper: " .. csssafe(o.paper)
    end
    if o.rule ~= "" then
        rules[#rules + 1] = "--rule: " .. csssafe(o.rule)
    end
    if o.step ~= "" then
        rules[#rules + 1] = "--step: " .. csssafe(o.step)
    end
    if #rules > 0 then
    ...
    local body_rules = {}
    if o.size ~= "" then
        body_rules[#body_rules + 1] = "font-size:" .. csssafe(o.size)
    end
    if o.leading ~= "" then
        body_rules[#body_rules + 1] = "line-height:" .. csssafe(o.leading)
    end
    if o.tracking ~= "" then
        body_rules[#body_rules + 1] = "letter-spacing:" .. csssafe(o.tracking)
    end
    if o.weight ~= "" then
        body_rules[#body_rules + 1] = "font-weight:" .. csssafe(o.weight)
    end
    if o.align ~= "" then
        body_rules[#body_rules + 1] = "text-align:" .. csssafe(o.align)
    end
    if #body_rules > 0 then
        out[#out + 1] = "<style>body{" .. table.concat(body_rules, ";") .. "}</style>\n"
    end
    if o.columns ~= "" then
        out[#out + 1] = "<style>article{column-count:"
            .. csssafe(o.columns)
    ...
    if o.scroll == "reverse" then
        bodycls = bodycls .. " scroll-reverse"
    end
    if o.scroll == "horizontal" then
        bodycls = bodycls .. " scroll-horizontal"
    end
    ...
    if bg ~= "" and bg ~= "none" then
        ...
        if bgchars ~= "" then
            bgattrs = bgattrs .. ' data-bgchars="' .. esc(bgchars) .. '"'
        end
        if bgopts ~= "" then
            bgattrs = bgattrs .. ' data-bgopts="' .. esc(bgopts) .. '"'
        end
    end
    ...
    if js ~= "" then
        out[#out + 1] = '<script src="/js/' .. js .. '.js"></script>\n'
    end
    if safeid(o.wasm) ~= "" then
        out[#out + 1] = '<script src="/js/wasm.js"></script>\n'
    end
    out[#out + 1] = "</body>\n</html>\n"
...
function render.home(d)
    ...
    out[#out + 1] = "<p>"
        .. #d.blocks
        ..
        " blocks. nothing here is finished. I am Adheesh/afish, i feel nihlist about things, this page consumes around 100 kbs, why our my ideas worth pages or bits, being hopecore is tough. But i think the proof of my humaniy is this maybe, style or taste is still human i think</p>\n"
...
function render.contradictions(d)
    ...
    for _, p in ipairs(d.pairs) do
        ...
        for _, side in ipairs({ p.left, p.right }) do
            out[#out + 1] = '<div class="side"><h3><a href="/b/'
                .. esc(f(side, "id"))
                .. '">'
                .. esc(f(side, "title"))
                .. "</a></h3>\n"
...
function render.dir(d)
    ...
    for _, e in ipairs(d.entries) do
        ...
        out[#out + 1] = '<li class="'
            ...
            .. "</a>"
        if not dir then
            out[#out + 1] = ' <span class="stage">' .. human(e.size) .. "</span>"
        end
        out[#out + 1] = "</li>\n"
    end
    if #d.entries == 0 then
        out[#out + 1] = "<li><em>empty</em></li>\n"
    end
    out[#out + 1] = "</ul>\n"
...
function render.mounts(d)
    ...
    d.entries = d.entries or {}
    for _, e in ipairs(d.entries) do
        out[#out + 1] = '<li class="dir"><a href="/mnt/'
            .. esc(f(e, "name"))
            .. '/">'
            .. esc(f(e, "name"))
            .. "/</a></li>\n"
...
function render.notfound(d)
    return page("nothing grows here", '<h1>nothing grows here</h1>\n<p class="stage">' .. esc(f(d, "path")) .. "</p>\n")
end
